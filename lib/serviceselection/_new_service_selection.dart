...
class _ServiceSelection extends State<ServiceSelection>
{
  ...
  void initState() {
    ...
    FirebaseFirestore.instance.collection("price").doc(service_name).get().then((value){
      setState(() {
        value[car_type] == null ? price = "0" : price = value[car_type];
      });
    });
             ...
             return Column(
                children: snapshot.data.docs.map((e){
                  ...
                  if(distance < 5000) {
                    ...
                    stream: FirebaseFirestore.instance.collection('services').doc(e.data()['userid']).collection("service").snapshots(),
                      builder: (BuildContext context,AsyncSnapshot<QuerySnapshot> snapshot) {
                        if (snapshot.hasData) {
                          return Column(
